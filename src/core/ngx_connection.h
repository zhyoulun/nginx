...
struct ngx_listening_s {//ngx_listening_t 结构体又代表着 Nginx 服务器监听的一个端口
    ngx_socket_t        fd;//socket 套接字句柄

    struct sockaddr    *sockaddr;//监听 sockaddr 地址
    socklen_t           socklen;    /* size of sockaddr */  //sockaddr 地址长度
    size_t              addr_text_max_len;//存储 IP 地址的字符串 addr_text 最大长度，即它指定了 addr_text 所分配的内存大小
    ...
    void               *servers;  /* array of ngx_http_in_addr_t, for example */ //实际上框架并不使用servers指针，它更多是作为一个保留指针，目前主要用于HTTP或者mail等模块，用于保存当前监听端口对应着的所有主机名
